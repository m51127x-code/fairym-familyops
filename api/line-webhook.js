...
const todayWeekday = weekdays[todayDate.getDay()];
  ...
  規則：
  1. content: 必須把所有「相對日期/時間詞彙」從文字中完全移除，包括：這週X、下週X、這禮拜X、這星期X、明天、後天、今天、昨天、下個月X號 等所有表示日期的詞。只保留動作描述與具體時間數字（如：10:30、下午三點）。
   ✅ 正確範例：「這週六10:30要去美甲」→ "10:30要去美甲"
   ✅ 正確範例：「明天下午買牛奶」→ "買牛奶"
   ❌ 錯誤範例：「這週六10:30要去美甲」→ "這週六10:30要去美甲"（沒有移除「這週六」）
